// File manages the login and signup pages. 
...
function initAuthUI() {
  ...
  signupForm?.addEventListener("submit", async (e) => {
    ...
    const username =
      document.querySelector("#user-username")?.value?.trim() ?? "";
    const age = document.querySelector("#user-age")?.value?.trim() ?? "";
    const email = document.querySelector("#signupEmail")?.value?.trim() ?? "";
    const password = document.querySelector("#signupPassword")?.value ?? "";
    if (!name || !email || !password) {
      showError(
        "Please fill in required fields: name, username, age, email, and password.",
      );
      return;
    }
